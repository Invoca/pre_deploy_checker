...
describe 'PushManager' do
  ...
  it 'can create jira issues, commits, and link them together' do
    ...
    push = PushManager.process_push!(jira_client, Push.create_from_github_data!(payload))
  ...
  context 'status' do
    ...
    context 'has a success status' do
      ...
      it 'when there is an accepted jira error' do
        ...
        record.save!
        push = PushManager.process_push!(jira_client, push)
        expect(push.status).to eq('success')
      end
  ...
  context 'uses appropriate ancestor branch' do
    it 'for default' do
      ...
      PushManager.process_push!(jira_client, Push.create_from_github_data!(payload))
    end
